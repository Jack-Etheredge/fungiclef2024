...
def train(generator, discriminator, data, batch_count, criterion, optimizerG, optimizerD, nz, device, real_label=1.,
          fake_label=0., label_smoothing_eps=0.0):
    ...
    critic_iter = 5
    optimizerD.zero_grad()
    real_labeled_real_embeddings = embeddings[labels == real_label]
    ...
    gradient_penalty = calc_gradient_penalty(discriminator, real, fake)
...
def train_openganfea(cfg: DictConfig) -> str:
    ...
    project_name += f"adambeta0.5_dlr_{lr_d:.0e}_glr_{lr_g:.0e}_open{openset_label}closed{closedset_label}_wgangp"
