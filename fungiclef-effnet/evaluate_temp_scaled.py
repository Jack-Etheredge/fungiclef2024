...
def make_submission(test_metadata, model_path, output_csv_path, images_root_path, temp_scaling):
    ...
    model = PytorchWorker(model_path, temp_scaling)
...
def evaluate_experiment(experiment_id, temperature_scaling=False):
    experiment_dir = Path("model_checkpoints") / experiment_id
    model_file = "model_with_temperature.pth" if temperature_scaling else "model.pth"
