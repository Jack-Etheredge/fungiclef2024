...
def make_submission(test_metadata, model_path, model_name, output_csv_path="./submission.csv",
    ...
    for obs, observation_df in tqdm(test_metadata.groupby("observation_id", sort=False)):
        ...
        try:
            ...
            obs_max = 0.0
            obs_argmax = -1
            for obs_pred in obs_preds:
                curr_max = np.max(obs_pred)
                if curr_max > obs_max:
                    obs_max = curr_max
                    obs_argmax = np.argmax(obs_pred)
            test_metadata.loc[test_metadata["observation_id"] == obs, "class_id"] = obs_argmax
...
if __name__ == "__main__":
    ...
    make_submission(
        ...
        output_csv_path="./submission_max.csv"
    )
